---
title: "Processing of Results from DADA2/DECIPHER Pipeline for HRE Sewage Microbiome Data"
author: "Liz Suter"
date: "7/11/2020"
---

I reran the raw sequence files in a DADA2 in pipeline using DECIPHER for classfication because they already has a classifier based on Silva v138 (QIIME does not have this yet. They are still working with v132).

References:
- [Gloor et al. 2017](https://www.frontiersin.org/articles/10.3389/fmicb.2017.02224/full).
- [McMurdie and Homes 2013](https://journals.plos.org/plosone/article?id=10.1371/journal.pone.0061217)
- [Silverman et al. 2017](https://elifesciences.org/articles/21887)
- [Coenan et al. 2020](https://www.frontiersin.org/article/10.3389/fgene.2020.00310/full)
- [Fernandes et al. 2014](https://microbiomejournal.biomedcentral.com/articles/10.1186/2049-2618-2-15)
- [Mandal et al. 2015](https://www.ncbi.nlm.nih.gov/pmc/articles/PMC4450248/)
  - There is an appended ANCOM method, ANCOMII, from [Kaul et al. 2017](https://www.ncbi.nlm.nih.gov/pmc/articles/PMC5682008/) for datasets with excessive zeroes.

Tutorials:
- [Happy Belly Ampicons workflow](https://astrobiomike.github.io/amplicon/dada2_workflow_ex)
- [Coenan et al. repo](https://github.com/WeitzGroup/analyzing_microbiome_timeseries)
- [phyloseq](https://joey711.github.io/phyloseq/)
- [DeSeq2 in phyloseq](http://joey711.github.io/phyloseq-extensions/DESeq2.html)
- [phILR](https://bioconductor.org/packages/release/bioc/vignettes/philr/inst/doc/philr-intro.html)
- [ALDEx2](https://www.bioconductor.org/packages/release/bioc/vignettes/ALDEx2/inst/doc/ALDEx2_vignette.pdf)
- [UC Davis 2017 Workshop](https://ucdavis-bioinformatics-training.github.io/2017-September-Microbial-Community-Analysis-Workshop/friday/MCA_Workshop_R/phyloseq.html)
- [Stat Analysis of Microbiome data](https://www.nicholas-ollberding.com/post/introduction-to-the-statistical-analysis-of-microbiome-data-in-r/)
- [ANCOM + ANCOMII](https://github.com/FrederickHuangLin/ANCOM)
- [microbiomeR](https://microbiome.github.io/tutorials/#microbiome_analysis) *Didn't use yet but look into it*
...
Metadata:
```{r}
metadata <- read_csv("sample-metadata_dada2.csv")
```
...
rarecurve(t(count_table_df)[1:5,], step=100, cex=0.5, ylab="ASVs", label=T)
```

Generally these seem to be sequenced to completion. A1 has the lowest sequencing depth but also starts to plateau so leave it in dataset for now.

### Remove singletons

```{r}
count_table_no_singletons <- filter(count_table,rowSums(count_table)>1)
# retains 27950 ASVs (out of 27964)
```



### Make Phylo Tree  
This process takes a LONG time (>24 HOURS) so run once and save .RData object
...
Since previous step took awhile to generate, save current variables in environment as RData object
```{r}
# save.image("HREmicrobiome_postanalysis_vars_upto_alignment.RData")
```
...
```{r}
# save.image("HREmicrobiome_analysis_vars_upto_tree.RData")
```

If variables are lost, come back and load everything up to now:
```{r}
load("/Volumes/MyPassport/HREMicrobiomeproject2019_LargeFiles/dada2_decipher_analysis_files/post_analysis_R/HREmicrobiome_analysis_vars_upto_tree.RData")
```
*Note on the above: I moved that Rdata object to my external storage since it is very large. Load it from there.*
...
Filter out those ambigious Domain annotations- I am losing 11,414 ASVs!
```{r}
...
Filter out any with "NA" as phylum. For now, I am leaving in phyla with low abundance 
```{r}
ps <- subset_taxa(ps, !is.na(phylum) & !phylum %in% c(""))

table(tax_table(ps)[, "phylum"], exclude = NULL)
```
After the above, 13,909 ASVs remain
...
# and plot
phyla_barplot <- plot_bar(phylumGlommed_RA, x = "Sample", fill = "phylum") + 
  scale_fill_manual(values = PhylaPalette) +
...
Check abundance plots for some phyla of interest
...
# and plot
proteobac_orders_barplot <- plot_bar(orderGlommed_RA, fill = "order") + 
  scale_fill_manual(values = OrderPalette)

proteobac_orders_barplot

# export
ggsave("figures/proteobac_orders_barplot.eps",proteobac_orders_barplot, width = 15, height = 5, units = c("in"))
...
```{r}
# subset 
ps_actino_ra <- subset_taxa(ps_ra, phylum == "Actinobacteriota")
...
# Then aglomerate the ASVs at the phylum level using the phyloseq function, tax_glom
actino_orderGlommed_RA = tax_glom(ps_actino_ra, "order")
# and plot
actino_orders_barplot <- plot_bar(actino_orderGlommed_RA, fill = "order") + 
  scale_fill_manual(values = OrderPalette)

actino_orders_barplot

ggsave("figures/actino_orders_barplot.eps",actino_orders_barplot, width = 15, height = 5, units = c("in"))
```


I could keep going with this. There are several other phyla that are abundant: Acidobacteriota, Bacteroidota, Bdellovibrionota, Campilobacterota, Chloroflexi, Cyanobacteria, Dependentiae, Desulfobacterota, Elusimicrobiota, Firmicutes, Myxococcota, Patescibacteria, Planctomycetota, Verrucomicrobiota.
