...
```{r}
SRARunTable <- read_csv("SRA_into_DADA2/SraRunTable.txt")
...
Make an alignment using tools from Decipher (Note- alignment step takes several hours. Commented out for now. Only need to run once) **RUNNING AT 9:52PM SUNDAY**
```{r}
## import fasta
...
```{r}
# save.image("CariacoEuks_postanalysis_vars_upto_alignment.RData")
```
...
```{r}
# save.image("CariacoEuks_postanalysis_vars_upto_tree.RData")
```
...
Filter out those ambigious Supergorup annotations- losing 471 ASVs
```{r}
